---
title: "WPILib Command Architecture: CommandXboxController, RunCommand"
sidebarTitle: "Commands"
description: "Reference for WPILib Command-Based classes used in FRC Academy: CommandXboxController button triggers, RunCommand inline lambdas, and InstantCommand one-shot execution."
---
...
When the button is released, WPILib interrupts the `whileTrue` command automatically, then immediately fires the `.onFalse()` command. The stop `RunCommand` runs once, sets the motor to `0.0`, and since nothing is keeping it running it ends on the next scheduler cycle. The result is a motor that runs at full speed exactly while the button is held and stops cleanly the moment the driver lets go.
