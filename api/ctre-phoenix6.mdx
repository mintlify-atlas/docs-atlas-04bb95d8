...
**4-Motor Tank Drive Example**

In FRC Academy's 4-motor Kraken lesson, each follower mirrors its side's lead motor. Right-side inversion is handled separately via `MotorOutputConfigs`, so both followers use `false` for `opposeLeaderDirection`:
